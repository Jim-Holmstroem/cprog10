...
namespace lab2{
    ...
    template<typename T>
      ...
      public:
        ...
        bool add_event(string,int); //day
        bool add_event(string,int,int); //day,month
        bool add_event(string,int,int,int); //day,month,year
        ...
        bool move_event(const Date&, const Date&, string);
        bool add_related_event(const Date&, int, string, string);
        //bool add_perm_event(const Date&, string); //use add_related_event multiple times, default recurrence 100 days
        //bool add_perm_event(const Date&, int, string); //occur in specified interval
        //bool add_perm_event(const Date&, skefjskefj); //occur on same date each year
        //bool add_perm_event(const Date&, string, string); //specify a weekday
        //bool add_bday(&add_event, string, const Date&); //string is name of person
    ...
    template<typename T>
        bool Calendar<T>::move_event(const Date& from, const Date& to, string event){
        bool rm = remove_event(event, from.day(), from.month(), from.year()); //false = does not exist
        bool add = add_event(event, to.day(), to.month(), to.year()); //false = already exists
        if( rm && add ){
	    return true;
	}else{ //either add or removed failed
            if( rm ){
		//add the removed event to restore previous state
                add_event(event, from.day(), from.month(), from.year());
            }
            if( add ){
		//restore previous state
                remove_event(event, to.day(), to.month(), to.year());
            }
            return false;
	}
    }
    template<typename T>
        bool Calendar<T>::add_related_event(const Date& rel_date, int days, string rel_event, string new_event){
    }
