...
hax::Level::Level(){
    ...
    opmap["a"] = new LevelOpVoid(&Level::action); //TODO remove after test
    ...
    opmap["pick up"] = new CharOpBool1String(&Character::pick_up);
    ...
    opmap["stats"] = new CharOpVoid(&Character::view_stats);
    opmap["view"] = new CharOpVoid(&Character::view_area);
    opmap["inv"] = new CharOpVoid(&Character::view_inventory);
    ...
    charHelp["pick up"] = "Pick up an object, pick up <object>";
    charHelp["drop"] = "Drop an object, drop <object>";
//  charHelp["cd"] = "Change viewed container, cd <container>, empty argument to return home";
    ...
    charHelp["inv"] = "Show the objects carried by the character";
}
...
void hax::Level::updatePlayers(){
    for(size_t i=0; i<vec_area.size(); i++){
        const std::vector<Character*> charsInArea = vec_area[i]->chars();
