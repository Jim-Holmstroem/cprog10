...
void Matrix::solve(){ //order of search: up, left, down, right
    std::size_t rownr = 0;
    std::size_t colnr = 0;
    ...
    recursion(rownr, colnr);
}

Matrix& Matrix::recursion(size_t rownr, size_t colnr){
    std::string dummy;
#ifdef DMAZE
    std::cout <<"in recursion: "<<"rownr="<< rownr <<" colnr="<< colnr << std::endl;
#endif
    std::cout << (*this) << std::endl;
    std::cin >> dummy;
    if((*this)[rownr][colnr] == 'E'){
        (*this)[rownr][colnr] = '.';
        return *this;
    }
    else{
        (*this)[rownr][colnr] = '.';
        if(rownr!=0 && (*this)[rownr-1][colnr] == 'E'){ //up
            return *this;
        }
	if(colnr!=0 && (*this)[rownr][colnr-1] == 'E'){ //left
            return *this;
        }
        if(rownr!=m_rows-1 && (*this)[rownr+1][colnr] == 'E'){ //down
            return *this;
        }
        if(colnr!=m_cols-1 && (*this)[rownr][colnr+1] == 'E'){ //right
            return *this;
        }
        if(rownr!=0 && (*this)[rownr-1][colnr] == ' '){ //up
            (*this).recursion(rownr-1, colnr);
            (*this)[rownr-1][colnr] = ' ';
        }
	if(colnr!=0 && (*this)[rownr][colnr-1] == ' '){ //left
            (*this).recursion(rownr, colnr-1);
            (*this)[rownr][colnr-1] = ' ';
        }
        if(rownr!=m_rows-1 && (*this)[rownr+1][colnr] == ' '){ //down
            (*this).recursion(rownr+1, colnr);
            (*this)[rownr+1][colnr] = ' ';
        }
        if(colnr!=m_cols-1 && (*this)[rownr][colnr+1] == ' '){ //right
            (*this).recursion(rownr, colnr+1);
            (*this)[rownr][colnr+1] = ' ';
        }
        return *this;
