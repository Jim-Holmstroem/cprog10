...
//	Vector<bool>* iterObject;
	unsigned int* curInt;
	int maskpos;

        const_iterator(){};
        const_iterator(unsigned int* data, const int maskpos){
            curInt = data;
            this->maskpos = maskpos;
        };
        const_iterator(const const_iterator& copy){
            curInt = copy.curInt;
            maskpos = copy.maskpos;
        };
...
//            return (*this)++; ??????
        ...
        const_iterator& operator+=(int pos){ //TODO
        };
        const_iterator& operator-=(int pos){ //TODO
        };
        ...
        reference operator*() const{
            return bool_proxy(curInt, maskpos);
        };
...
//      typedef const bool_proxy& const_reference;
      ...
      iterator(unsigned int* data, const int maskpos) : const_iterator(data, maskpos){};
...
// ostream& operator ostream(){ ???????????
    ...
    const_iterator begin() const{
        return const_iterator(&elems[0], 0);
    };
