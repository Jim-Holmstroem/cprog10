...
//unsigned int counter[] ={NUMBER_OF_VECTORS,NUMBER_OF_VECTORS,NUMBER_OF_VECTORS,NUMBER_OF_VECTORS,NUMBER_OF_VECTORS,NUMBER_OF_VECTORS,NUMBER_OF_VECTORS,NUMBER_OF_VECTORS};
    ...
    std::cout << "test change value through operator[]/*" << std::endl;
    for(int i=0; i<=35; i++){
        it[i] = true;
    }
    v1.print();
...
//    std::cout << *(it+31) << *(it+32) << *(it+33) << *(it+34) << std::endl; //TODO weird output when ostreamed like this and not using endl
    ...
    Vector<bool>::const_iterator itc = v.begin();
    std::advance(itc, 3);
