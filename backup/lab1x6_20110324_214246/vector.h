...
//            data = boolvec->getUintData(absIndex);
    ...
    class const_iterator{
      public:
	    ...
	    return const_iterator(this->iterObj, this->absIndex++);
        };
        const_iterator& operator--(){
            return (*this) += -1;
        };
        ...
        const_iterator operator+(int pos) const{
            return const_iterator(this->iterObj, this->absIndex + pos);
        };
        const_iterator operator-(int pos) const{
            return (*this) + -pos;
        };
        ...
        difference_type operator-(const const_iterator& iter) const{ //TODO test
        ...
        reference operator*() const{
            return bool_proxy(this->iterObj, this->absIndex);
        };
        reference operator[](int pos) const{ //TODO
            return (*iterObj)[pos];
//          return bool_proxy(this->iterObj, pos);
        };
...
//      typedef const bool_proxy const_reference;
      ...
      iterator(const iterator& copy) : const_iterator(copy){};
        ...
        iterator operator++(int){ //TODO is this always a copy?
//            ++this;
//            return (*this)++; ??????
        };
        iterator& operator--(){
        ...
        iterator operator+(int pos) const{
            return iterator(this->iterObj, this->absIndex + pos);
        };
        iterator operator-(int pos) const{
            return (*this) + -pos;
        };
        ...
        reference operator*() const{ //reference so it can be changed through iterator if dereferenced
