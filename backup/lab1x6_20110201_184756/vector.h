  ...
  public:
    ...
    class const_iterator : public std::iterator<std::random_access_iterator_tag, bool>{
      public:
        ...
        const_iterator(){};
        const_iterator(unsigned int* const ptr){ //read-only pointer
//          curElem = ptr;
        };
//      unsigned int* const curElem;
    };
    ...
    class iterator : public const_iterator{
      public:
        ...
        int maskpos;
        bool_proxy bp;
        iterator(unsigned int* data, const int maskpos){
            curInt = data;
            this->maskpos = maskpos;
        };

        iterator(bool_proxy bp){
            this->bp = bp;
        };

      iterator(const iterator& iter) : curInt(iter.curInt), maskpos(iter.maskpos){
        };
        iterator& operator=(const iterator& iter){
            curInt = iter.curInt;
            maskpos = iter.maskpos;
            return *this;
        };

        bool_proxy& operator*(){ //reference so it can be changed through iterator if dereferenced
	        bp = bool_proxy(curInt, maskpos);
	        return bp;
        };
//	bool_proxy& operator->(){

        iterator& operator++(){ //move these operators to class bool_proxy instead? SIMON
            maskpos++;
            if(maskpos >= constant::BOOL_PER_INT){
                maskpos = 0;
                curInt++;
            }
//            boolElem = bool_proxy(curInt, maskpos);
//          curElem += (float)1/constant::BOOL_PER_INT;
            return *this;
        };
...
//            return (*this)++; ??????
        ...
        iterator& operator--(){
            if(maskpos <= 0){
                maskpos = constant::BOOL_PER_INT-1;
                curInt--;
            }else{
                maskpos--;
            }
        ...
        iterator& operator+(const int pos){
            maskpos += pos;
            if(maskpos >= constant::BOOL_PER_INT){
                curInt += maskpos/(constant::BOOL_PER_INT);
                maskpos = maskpos%(constant::BOOL_PER_INT);
            }
            return *this;
        };
	iterator& operator-(const int pos){ //SIMON testa!!!!
	    maskpos -= pos;
	    while(maskpos < 0){
		curInt--;
		maskpos += constant::BOOL_PER_INT;
	    }
	    return *this;
	};

        bool operator==(const iterator& iter) const{
            if(this->curInt == iter.curInt && this->maskpos == iter.maskpos){
                return true;
            }else{
                return false;
            }
...
/*    iterator begin(){
      return iterator(bool_proxy(&elems[0],0), 0); //use instead make_bool_proxy(0)????
      };
      unsigned int* end(){
      return &elems[size()-1]+sizeof(unsigned int); //SIMON
      };
*/
