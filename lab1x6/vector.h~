...
#include<iterator> //TODO not needed?
...
class Vector<bool>{
  ...
  public:
    typedef const bool& const_reference; //TODO should this be here?
...
//            data = boolvec->getUintData(absIndex);
    ...
    class const_iterator{
      ...
      public:
        ...
        typedef std::ptrdiff_t difference_type; //gives error no match for operator- in last to __first
...
//      typedef const bool_proxy const_reference;
	...
	//difference operator
        ...
        reference operator*() const{ //bool_proxy so it can be changed if dereferenced
            return bool_proxy(iterObj, absIndex);
        };
        reference operator[](int pos) const{
            return (*iterObj)[pos];
